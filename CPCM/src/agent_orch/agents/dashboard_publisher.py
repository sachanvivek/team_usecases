...
def launch_dashboard():
    ...
    def fetch_server_options(api_base_url: str) -> List[str]:
    ...
    if api_base_clean:
        try:
            server_options = fetch_server_options(api_base_clean)
        except Exception as exc:
            st.warning(f"Could not load server list: {exc}")

    server_choices = [""] + server_options
    if st.session_state["active_server"] not in server_choices:
    ...
    resolved_server = server_select.strip()
    ...
    if approval_result:
        ...
        col_left.metric("Scaling Executed", str(approval_result.get("scaling_executed")))
        ...
        if approval_result.get("scaling_result"):
            st.markdown("#### Scaling Result")
            st.json(approval_result["scaling_result"])

        if approval_result.get("dashboard"):
            st.success(approval_result["dashboard"])
